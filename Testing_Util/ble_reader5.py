...
def notification_handler(sender, data):
    ...
    # Ensure we have the full 113-byte payload (8 samples * 14 bytes + 1 event byte)
    if len(data) == 113:
        ...
        # Extract the event marker byte (the last byte in the payload at index 112)
        event_marker = data[112]
        event_str = "EVENT" if event_marker != 0 else ""

        # Loop through the first 112 bytes in 14-byte chunks
        for i in range(8):
            # Slice out the current 14-byte chunk
            chunk = data[i * 14 : (i + 1) * 14]

            # Unpack 6 signed 16-bit integers and 1 unsigned 16-bit integer (Little Endian)
            ax, ay, az, gx, gy, gz, device_ts = struct.unpack("<6hH", chunk)

            # Write directly to the CSV file, adding device_ts and event_str
            file_handle.write(f"{int(elapsed_ms)},{ax},{ay},{az},{gx},{gy},{gz},{device_ts},{event_str}\n")
...
async def main():
    ...
    with open(OUTPUT_FILENAME, "w") as f:
        ...
        # Write CSV Headers
        file_handle.write(
            "ms_since_start,accel_x,accel_y,accel_z,gyro_x,gyro_y,gyro_z,device_timestamp,event\n"
        )
